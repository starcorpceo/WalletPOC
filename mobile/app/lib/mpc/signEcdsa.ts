      ...
      switch (signStatus) {
        ...
        case "Stepping":
          if (isValidStart(message)) {
            initSignEcdsa(new Uint8Array(Buffer.from(messageToSign, encoding as BufferEncoding)), clientShare).then(
              (success) => {
                success && step(null).then((stepMsg) => websocket.send(stepMsg.message));
              }
            );
            return;
          }
