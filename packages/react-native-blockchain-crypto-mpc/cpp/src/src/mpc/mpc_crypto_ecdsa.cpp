...
{
  error_t rv = 0;
  if (rv = refresh.peer2_step1(share, in, out))
    return rv;
  return 0;
}
...
{
  error_t rv = 0;
  if (rv = refresh.peer1_step2(share, in, out))
    return rv;
  return 0;
}
...
{
  error_t rv = 0;
  if (rv = refresh.peer2_step2(share, in))
    return rv;
  return 0;
}
...
{
  error_t rv = 0;
  if (rv = agree_random.peer2_step1(in, out))
    return rv;
  return 0;
}
...
error_t mpc_ecdsa_gen_t::party1_step2(const message2_t &in, message3_t &out)
{
  error_t rv = 0;
  ub::buf_t session_id;
  if (rv = agree_random.peer1_step2(in, out.agree_msg3, session_id))
    return rv;
...
error_t mpc_ecdsa_gen_t::party2_step2(const message3_t &in, message4_t &out)
{
  ...
  if (rv = agree_random.peer2_step2(in.agree_msg3, session_id))
    return rv;
  if (rv = ctx.peer2_step1(false, in.curve, session_id, share, in.gen_msg1, out))
    return rv;
  curve = in.curve;
  return rv;
}
...
{
  error_t rv = 0;
  if (rv = ctx.peer1_step2(share, in, out))
    return rv;
  return rv;
}
...
{
  error_t rv = 0;
  if (rv = ctx.peer2_step2(share, in))
    return rv;
  return rv;
}
...
{
  error_t rv = 0;
  if (in.data_to_sign != ctx.data_to_sign)
    return rv = ub::error(E_BADARG);
  if (in.refresh != ctx.refresh)
    return rv = ub::error(E_BADARG);

  if (rv = ctx.peer2_step1(share, in.data_to_sign, in.refresh, in.sign_msg1, out))
    return rv;
  return rv;
}
...
{
  error_t rv = 0;
  if (rv = ctx.peer2_step2(share, in, out))
    return rv;
  return rv;
}
...
{
  error_t rv = 0;
  if (rv = ctx.peer1_step3(share, in, out))
    return rv;
...
{
  error_t rv = 0;
  if (rv = ctx.peer2_step3(share, in))
    return rv;
  return rv;
}
...
MPCCRYPTO_API int MPCCrypto_getResultEcdsaSign(MPCCryptoContext *context, uint8_t *signature, int *out_size, bool toDer)
{
  error_t rv = 0;
  ...
  buf_t der = toDer ? ctx->get_signature().to_der() : ctx->get_signature().to_bin();
  *out_size = der.size();
  if (signature)
  {
