...
#define AES_EXPAND_ASSIST(v1, v2, v3, v4, shuff_const, aes_const)   \
  ...
  {
    if (mode_aesni)
      return aesni.encrypt(in);
    else
      return openssl.encrypt(in);
  }
  ...
  {
    if (mode_aesni)
      return aesni.encrypt(src1, dst1, src2, dst2, src3, dst3, src4, dst4);
    else
      return openssl.encrypt(src1, dst1, src2, dst2, src3, dst3, src4, dst4);
  }

  void aes_enc128_t::encrypt(buf128_t src1, buf128_t &dst1, buf128_t src2, buf128_t &dst2) const
  {
    if (mode_aesni)
      return aesni.encrypt(src1, dst1, src2, dst2);
    else
      return openssl.encrypt(src1, dst1, src2, dst2);
  }
