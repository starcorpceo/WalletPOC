...
void gcdef_bip_t::set_const_4(wires_t &dst, int offset, unsigned value)
{
  ...
  for (int i = 0; i < 32; i++)
    dst[offset + i] = get_const_wire(get_bit(buffer, i));
}
...
circuit_def_t::wires_t gcdef_bip_t::hmac(wires_t &opad_state, wires_t &ipad_state, wires_t &in)
{
  ...
  for (int i = data_size * 8; i < block_count * block_size * 8; i++)
    hmac_input[i] = get_wire_false();
  ...
  for (int i = 0; i < block_count; i++)
  {
    ...
    for (int j = 0; j < block_size * 8; j++)
      block[j] = hmac_input[i * block_size * 8 + j];
  ...
  for (int i = 0; i < state_size * 8; i++)
    block[i] = out_state[i];
  for (int i = state_size * 8; i < block_size * 8; i++)
    block[i] = get_wire_false();
...
gcdef_bip_t::gcdef_bip_t(int initial_seed_size, unsigned index)
{
  ...
  if (initial)
  {
    ...
    for (int i = 0; i < q_size; i++)
      prev_x[i] = get_wire_false(); // all zeroes
  }
  else
  {
    ...
    for (int i = 0; i < 8; i++)
      hmac_in[i] = get_wire_false(); // first zero byte

    prev_x = add_mod(in1, in2, q);
    wires_t prev_x_swap = prev_x;
    prev_x_swap.swap_bytes();
    for (int i = 0; i < q_size; i++)
      hmac_in[8 + i] = prev_x_swap[i];
...
error_t mpc_ecdsa_derive_bip_t::init(bool hardened, unsigned index, const mpc_ecdsa_share_t &ecdsa_share)
{
  ...
  if (!hardened)
  {
    if (rv = execute_normal_derivation())
      return rv;
  }
...
{
  error_t rv = 0;
  if (rv = ot_base_init_sender.snd_step2(ot_sender, in.ot_base_init1_msg1, out.ot_base_init1_msg2))
    return rv;                                                          // ot_base_init1
...
static void set_rho(const gcdef_bip_t *circuit_def, gc_plain_t &input, const std::vector<bn_t> &rho)
{
  for (int i = 0; i < bip_sec_count + 1; i++)
  {
    buf_t rho_data = rho[i].to_bin(32);
    rho_data.reverse();
...
void mpc_ecdsa_derive_bip_t::init_circuit_def()
{
  ...
  if (hardened)
    child_index_fix |= 0x80000000;

  if (!circuit_def)
    circuit_def = cache_gcdef_bip_t::get(initial ? old_seed_share.size() : -1, initial ? 0 : child_index_fix);
  if (!mpc_circuit_def)
  {
...
void mpc_ecdsa_derive_bip_t::gc_init_peer1()
{
  ...
  else
  {
    ...
    buf_t prev_x = old_ecdsa_share.x.to_bin(32);
    prev_x.reverse();
...
error_t mpc_ecdsa_derive_bip_t::party1_step2(const message2_t &in, message3_t &out) // step 3
{
  ...
  if (rv = ot_base_init_sender.snd_step2(ot_sender, in.ot_base_init2_msg1, out.ot_base_init2_msg2))
    return rv;                                                                   // ot_base_init2
  ot_extend_receiver.rec_step1(max_ot_blocks, ot_receiver, out.ot_extend1_msg1); // ot_extend1

  gc_init_peer1();
  if (rv = gc.peer1_step1(out.gc_msg1))
    return rv;
...
error_t mpc_ecdsa_derive_bip_t::party2_step2(const message3_t &in, message4_t &out) // step 4
{
  ...
  if (rv = ot_extend_sender.snd_step2(max_ot_blocks, ot_sender, in.ot_extend1_msg1))
    return rv; // ot_extend1

  ot_extend_receiver.rec_step1(max_ot_blocks, ot_receiver, out.ot_extend2_msg1); // ot_extend2
  gc_init_peer2();
  if (rv = gc.peer2_step2(in.gc_msg1, out.gc_msg2))
    return rv;
...
error_t mpc_ecdsa_derive_bip_t::party1_step3(const message4_t &in, message5_t &out) // step 5
{
  ...
  if (rv = ot_extend_sender.snd_step2(max_ot_blocks, ot_sender, in.ot_extend2_msg1))
    return rv; // ot_extend2
  if (rv = gc.peer1_step3(in.gc_msg2, out.gc_msg3))
    return rv;
  if (rv = agree_random.peer2_step1(in.agree_msg1, out.agree_msg2))
    return rv;
...
static std::vector<bn_t> get_out_x(const int *param_tab, const gc_plain_t &output)
{
  ...
  for (int i = 0; i < bip_sec_count + 1; i++)
  {
    x_data = output.get_param(param_tab[i]);
    x_data.reverse();
...
{
  error_t rv = 0;
  if (rv = agree_random.peer1_step2(in.agree_msg2, out.agree_msg3, session_id))
    return rv;
  if (rv = gc.peer2_step4(in.gc_msg3, out.gc_msg4))
    return rv;
...
error_t mpc_ecdsa_derive_bip_t::check_new_Q()
{
  ...
  for (int i = 1; i < bip_sec_param + 1; i++)
  {
    ...
    if (Q_sum_hi == Q_hat && Q_sum_lo == Q_hat)
      continue;
...
{
  error_t rv = 0;
  if (rv = agree_random.peer2_step2(in.agree_msg3, session_id))
    return rv;
  if (rv = gc.peer1_step5(in.gc_msg4, out.gc_msg5))
    return rv;
...
error_t mpc_ecdsa_derive_bip_t::party2_step4(const message7_t &in, message8_t &out) // step 8
{
  ...
  if (rv = gen_helper.peer2_step1(true, curve, session_id, new_share, in.gen_msg1, out.gen_msg2))
    return rv;
...
error_t mpc_ecdsa_derive_bip_t::party1_step5(const message8_t &in, message9_t &out) // step 9
{
  ...
  if (!mpc::commitment_t::check(in.comm_Q2_rand, comm_Q2_hash, sha256))
    return rv = ub::error(E_CRYPTO);
...
MPCCRYPTO_API int MPCCrypto_getResultDeriveBIP32(MPCCryptoContext *context, MPCCryptoShare **new_share_ptr)
{
  ...
  if (rv = derive->get_result_share(new_share))
    return rv;
...
MPCCRYPTO_API int MPCCrypto_serializePubBIP32(MPCCryptoShare *share_ptr, char *out, int *out_size, bool main)
{
  ...
  if (bip_key_info.hardened)
    bip_key_info.child_number |= 0x80000000;
